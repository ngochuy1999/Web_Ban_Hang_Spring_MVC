...
public class ProductServiceImpl implements ProductService {
	...
	public List<Product> getNewProduct() {
		Session session = factory.getCurrentSession();
		try {
			String hql = "FROM Product r WHERE r.productstatus.id = 1 ORDER BY r.id DESC";
			Query query = session.createQuery(hql);
	...
	public List<Product> getProductByBrand(int id) {
		Session session = factory.getCurrentSession();
		try {
			String hql = "FROM Product r WHERE r.brand.id = :id AND r.productstatus.id = 1";
			Query query = session.createQuery(hql);
			query.setParameter("id", id);
			List<Product> result = query.list();
			if (result != null) {
	...
	public boolean isExitName(String name) {
		...
		Transaction t = session.beginTransaction();
		try {
			String hql = "SELECT count(p.id) FROM Product p WHERE p.name=:name";
			Query query = session.createQuery(hql);
